...
namespace hft::order_book_engine
{
    ...
    class HftOrderBookEngine
    {
      ...
      public:
        HftOrderBookEngine(FixMessagePacketQueue &queue, hft::monitoring::TelemetryCounters &telemetry,
                           hft::protocol::ProtocolType protocol_type, int cpu_pin = -1,
                           std::string log_filename = "hft_order_book_engine.log") noexcept
            : m_queue(queue), m_telemetry(telemetry), m_protocol_type(protocol_type), m_cpu_pin(cpu_pin),
              m_log_filename(std::move(log_filename)), m_order_book("PETR4")
        {
        ...
        {
            return m_order_book.total_trades();
        }
        [[nodiscard]] uint64_t total_volume() const noexcept
        {
            return m_order_book.total_volume();
        }
