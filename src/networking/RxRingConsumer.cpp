...
namespace hft::networking
{
    ...
    void PacketMmapRxConsumer::run()
    {
        ...
        if (m_cpu_pin >= 0)
        {
            ...
            hft::common::log_info("[RxRingConsumer] Thread pinned to CPU core " + std::to_string(m_cpu_pin));
        }
        ...
        if (m_sockfd < 0)
        {
            hft::common::log_warn("[RxRingConsumer] Note: socket(AF_PACKET) requires ROOT. Falling back to standard AF_INET UDP socket...");
            ...
            hft::common::log_info("[RxRingConsumer] Standard UDP Consumer bound to port " + std::to_string(m_filter_port));
        }
